...
namespace BayatGames.SaveGamePro.PlayMaker.Actions
{
    ...
    public class SaveGameWebLoadVector2Array : FsmStateAction
    {
        ...
        IEnumerator DoLoad()
        {
            ...
            if (!this.identifier.IsNone && !string.IsNullOrEmpty(this.identifier.Value))
            {
                ...
                {
                    var array = web.Load<UnityEngine.Vector2[]>();
                    this.valueArray.Values = Array.ConvertAll<UnityEngine.Vector2, object>(array, (item) =>
                    {
                        return (object)item;
                    });
