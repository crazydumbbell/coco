...
namespace BayatGames.SaveGamePro.PlayMaker.Actions
{
    ...
    public class SaveGameWebLoadVector3Array : FsmStateAction
    {
        ...
        IEnumerator DoLoad()
        {
            ...
            if (!this.identifier.IsNone && !string.IsNullOrEmpty(this.identifier.Value))
            {
                ...
                {
                    var array = web.Load<UnityEngine.Vector3[]>();
                    this.valueArray.Values = Array.ConvertAll<UnityEngine.Vector3, object>(array, (item) =>
                    {
                        return (object)item;
                    });
