...
namespace BayatGames.SaveGamePro.PlayMaker.Actions
{
    ...
    public class SaveGameWebLoadQuaternionArray : FsmStateAction
    {
        ...
        IEnumerator DoLoad()
        {
            ...
            if (!this.identifier.IsNone && !string.IsNullOrEmpty(this.identifier.Value))
            {
                ...
                {
                    var array = web.Load<UnityEngine.Quaternion[]>();
                    this.valueArray.Values = Array.ConvertAll<UnityEngine.Quaternion, object>(array, (item) =>
                    {
                        return (object)item;
                    });
