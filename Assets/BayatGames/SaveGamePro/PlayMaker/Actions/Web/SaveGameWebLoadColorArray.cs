...
namespace BayatGames.SaveGamePro.PlayMaker.Actions
{
    ...
    public class SaveGameWebLoadColorArray : FsmStateAction
    {
        ...
        IEnumerator DoLoad()
        {
            ...
            if (!this.identifier.IsNone && !string.IsNullOrEmpty(this.identifier.Value))
            {
                ...
                {
                    var array = web.Load<UnityEngine.Color[]>();
                    this.valueArray.Values = Array.ConvertAll<UnityEngine.Color, object>(array, (item) =>
                    {
                        return (object)item;
                    });
